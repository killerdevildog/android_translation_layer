...
/* include this before sysdeps because it needs `close`.
 * using the system `close` seems to only be a problem on Windows */
#include <base/file.h>
...
void restart_tcp_service(int fd, void *cookie) {
    ...
    setenv("ADBD_TCP_PORT", value, 1);
    WriteFdFmt(fd, "restarting in TCP mode port: %d\n", port);
    adb_close(fd);
}

void restart_usb_service(int fd, void *cookie) {
    setenv("ADBD_TCP_PORT", "0", 1);
