...
std::unique_ptr<RuntimeParser> ParsedOptions::MakeParser(bool ignore_unrecognized) {
  ...
  parser_builder->
      ...
      .Define("-Xbootclasspath/a:_")
          .WithType<std::string>()
          .IntoKey(M::BootClassPath_a)
...
bool ParsedOptions::DoParse(const RuntimeOptions& options,
  ...
  // if we didn't get bootclasspath via env either, use the default (seems weird to hardcode this, but openjdk does it as well)
  if(!args.Exists(M::BootClassPath)) {
    ...
    dladdr(art_sigsegv_fault, &libart_so_dl_info);
    ...
    if(libart_so_dl_info.dli_fname) {
      ...
      char *libart_so_full_path = malloc(strlen(libart_so_dl_info.dli_fname) + strlen(path_to_jars) + 1); // overallocating but that's fine
      ...
      *strrchr(libart_so_full_path, '/') = '\0'; // now we should have something like /usr/lib64/art
      ...
      char *bootclasspath_prefix = realpath(libart_so_full_path, NULL);
      free(libart_so_full_path);
      std::string bootclasspath_final = (std::string)
                                        bootclasspath_prefix + "/core-oj-hostdex.jar"      + ':' +
                                        bootclasspath_prefix + "/apachehttp-hostdex.jar"   + ':' +
                                        bootclasspath_prefix + "/apache-xml-hostdex.jar"   + ':' +
                                        bootclasspath_prefix + "/bouncycastle-hostdex.jar" + ':' +
                                        bootclasspath_prefix + "/core-junit-hostdex.jar"   + ':' +
      ...
      if(args.Exists(M::BootClassPath_a))
        bootclasspath_final = bootclasspath_final + ':' + *args.Get(M::BootClassPath_a);

      args.SetIfMissing(M::BootClassPath, ParseStringList<':'>::Split(bootclasspath_final));
    } else {
      fprintf(stderr, "bootclasspath wasn't provided on cmdline, wasn't provided by env, and we failed "
                      "to get the full path to libart.so in order to construct the default bootclasspath;\n"
                      "expect a failure when trying to run something\n");
    }
  }
  ...
  if (!args.Exists(M::CompilerCallbacksPtr) && !args.Exists(M::Image)) {
    ...
    if(!android_root) {
      ...
      if(libart_so_dl_info.dli_fname) {
        ...
        *strrchr(libart_so_full_path, '/') = '\0'; // now we should have something like /usr/lib64/art
        image += libart_so_full_path;
        image += "/../java/dex/art/oat/boot.art";
        free(libart_so_full_path);
      } else {
        image += "/tmp";
      }
    } else { // if ANDROID_ROOT is set, don't clobber the home directory
      image += android_root;
...
void ParsedOptions::Usage(const char* fmt, ...) {
  ...
  UsageMessage(stream, "  -Xbootclasspath/a:<additional bootclasspath entries> (NOTE: only used when -Xbootclasspath:bootclasspath is not specified)\n");
