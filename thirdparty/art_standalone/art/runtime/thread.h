...
class Thread {
 ...
 private:
  ...
  static __attribute__((visibility("default"))) bool is_started_;

  // TLS key used to retrieve the Thread*.
  static __attribute__((visibility("default"))) pthread_key_t pthread_key_self_;

  // Used to notify threads that they should attempt to resume, they will suspend again if
  // their suspend count is > 0.
  static __attribute__((visibility("default"))) ConditionVariable* resume_cond_ GUARDED_BY(Locks::thread_suspend_count_lock_);
