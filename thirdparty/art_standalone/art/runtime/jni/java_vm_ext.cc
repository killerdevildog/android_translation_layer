...
#include "../../bionic_translation/linker/dlfcn.h"
#include <string_view>
...
class SharedLibrary {
 public:
  ...
  void* FindSymbolWithoutNativeBridge(const std::string& symbol_name)
    ...
    return bionic_dlsym(handle_, symbol_name.c_str());
  }
...
bool JavaVMExt::LoadNativeLibrary(JNIEnv* env,
  ...
  if (android::NativeBridgeIsSupported(path_str)) {
    ...
    VLOG(jni) << "[Call to android::NativeBridgeLoadLibrary(\"" << path << "\", RTLD_NOW) returned " << handle << "]";
  } else {
    handle = bionic_dlopen(path_str, RTLD_NOW);
    VLOG(jni) << "[Call to bionic_dlopen(\"" << path << "\", RTLD_NOW) returned " << handle << "]";
  }

  if (handle == nullptr) {
    *error_msg = bionic_dlerror();
    VLOG(jni) << "bionic_dlopen(\"" << path << "\", RTLD_NOW) failed: " << *error_msg;
    return false;
  }
...
extern "C" __attribute__((visibility("default"))) jint JNI_CreateJavaVM(JavaVM** p_vm, JNIEnv** p_env, void* vm_args) {
...
extern "C" __attribute__((visibility("default"))) jint JNI_GetCreatedJavaVMs(JavaVM** vms_buf, jsize buf_len, jsize* vm_count) {
  Runtime* runtime = Runtime::Current();
...
extern "C" __attribute__((visibility("default"))) jint JNI_GetDefaultJavaVMInitArgs(void* /*vm_args*/) {
  return JNI_ERR;
}
