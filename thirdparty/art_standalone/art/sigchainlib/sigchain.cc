...
extern "C" int sigchain_sigaction(int signal, const struct sigaction* new_action,
...
extern "C" sighandler_t sigchain_signal(int signo, sighandler_t handler) {
  InitializeSignalChain();
...
#if !defined(__LP64__)
extern "C" sighandler_t sigchain_bsd_signal(int signo, sighandler_t handler) {
  InitializeSignalChain();

  return sigchain_signal(signo, handler);
}
#endif
...
extern "C" int sigchain_sigprocmask(int how, const sigset_t* new_set,
                           sigset_t* old_set) {
  InitializeSignalChain();
