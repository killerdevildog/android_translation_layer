...
JNIEXPORT jint JNICALL Java_com_wolfssl_WolfSSLSession_getPeerCertificateNum
  (JNIEnv* jenv, jobject jcl, jlong sslPtr)
{
    ...
    jint count = wolfSSL_get_chain_count(chain);
    if (count == 0 && wolfSSL_get_peer_certificate(ssl)) {
        // WolfSSL was likely build without `--enable-sessioncerts`, but we always have the leaf certificate
        count = 1;
    }
    return count;
#else
  ...
  (JNIEnv* jenv, jobject jcl, jlong sslPtr, jint index)
{
    ...
    if (index == 0) {  // the leaf certificate can be too large to fit in WOLFSSL_X509_CHAIN in some cases, so get it separately
